...
# API Key Information
# Because personal information is needed, there are blanks below 2 lines.
YT_API_KEY_BACK_UP = ""
DL_API_KEY = ""
...
def dl_trans(msg):
    ...
    data = requests.post(
        'https://api.deepl.com/v2/translate', data=params).json()

    return(data['translations'][0]['text'])

# reffering this method
...
def get_chat(chat_id, pageToken, f_today, yt_api, cnt):
    ...
    params = {'key': yt_api, 'liveChatId': chat_id,
    ...
    with open(f_today, 'a', encoding='utf-8') as f:
        ...
        try:
            for item in data['items']:
                ...
                c_time = dateutil.parser.parse(item['snippet']['publishedAt']).astimezone(
                ...
                if item.get('superChatDetails'):
                    ...
                    print(
                        f'{usr}さんからスパチャが来ました！ナイスパ！！ : {usr} Thanks for superchat!')
        ...
        # when error occurs, throw the reasons
        except:
            traceback.print_exc()
...
def main(yt_url, yt_api):
    ...
    while(chat_id):
        try:
            nextPageToken = get_chat(
                chat_id, nextPageToken, f_today, yt_api, count)
            time.sleep(slp_time)
        except:
            break
        count += 1
    ...
    while(chat_id):
        try:
            nextPageToken = get_chat(
                chat_id, nextPageToken, f_today, YT_API_KEY_BACK_UP, count2)
            time.sleep(slp_time)
